...
class AECCSensor(CoordinatorEntity, SensorEntity):
    ...
    def native_value(self):
        # Fehlerbehebung: Daten dynamisch aus dem Coordinator laden statt aus dem statischen Start-Objekt
        if not self.coordinator.data:
            return 0.0
            
        raw_data = self.coordinator.data.get(self._data_type)
        if not raw_data:
            return 0.0

        if isinstance(raw_data, list):
            # Für Listenstrukturen (Plugs, Charger, Heaters) das passende Gerät über die Seriennummer finden
            target_sn = self._item.get("PlugSN") or self._item.get("ChargerSN") or self._item.get("HotSN")
            current_item = None
            for item in raw_data:
                sn = item.get("PlugSN") or item.get("ChargerSN") or item.get("HotSN")
                if sn == target_sn:
            ...
            # Für einfache Strukturen wie SSumInfoList
            current_item = raw_data

        if not current_item:
            return 0.0

        value = current_item.get(self._path)
